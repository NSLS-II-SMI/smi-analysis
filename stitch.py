...
def stitching_giwaxs(datas, ais):
    ...
    for i, (data, ai) in enumerate(zip(datas, ais)):
        ...
        if i == 0:
            qimg = np.zeros((np.shape(qimage)[0], np.shape(qimage)[1], len(datas)))
            q_p, q_z = np.zeros((np.shape(qp)[0], len(datas))), np.zeros((np.shape(qz)[0], len(datas)))
    ...
    img_fin = img_te / sca2
    qp = [-qp_remesh.max(), -qp_remesh.min()]
    qz = [-qz_remesh.max(), -qz_remesh.min()]
    return img_fin, qp, qz


def stitching_waxs(datas, ais):
